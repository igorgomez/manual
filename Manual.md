...
Figura 1.4 - Tela de acesso ao Sistema com PEC
...
Figura 1.3 - Tela de acesso aos perfis disponíveis ao usuário do sistema
...
> ![](media/image74.png) **ATUALIZAÇÃO:** Em instalações do tipo Multimunicipal os prontuários clínicos poderão ser compartilhados entre as instâncias municipais da instalação. Disponível a partir da versão 3.2.
...
3.No combo box "Desabilitar durante", selecione uma das opções:

- 30 minutos, 1 hora, 2 horas, 4 horas, 8 horas ou "Tempo  indeterminado".

4.Clique no botão "Desabilitar" para concluir.
...
## 3.1.6 Configurações iniciais da RNDS

O PEC do e-SUS APS agora está integrado com a Rede Nacional de Dados em Saúde (RNDS). Esta integração permite que, durante um atendimento, os profissionais acessem dados de saúde do cidadão que foram registrados em outros estabelecimentos de saúde. Nesta versão nenhum dado registrado no PEC será enviado para a RNDS.

Para iniciar a configuração desta integração o administrador da instalação deve seguir seguintes passos:

- Obter um Certificado Digital válido;

- O **administrador da instalação** cria a pasta \"chaves\", dentro da pasta de instalação do e-SUS APS e adiciona o certificado digital (.pfx) do município. Dependendo da plataforma a ser utilizada, o certificado deverá ser guardado da seguinte forma:

**Windows**

![](media/image108.png)

Incluir o certificado digital (arquivo .pfx) com a chave na pasta
...
Para criar a pasta, é utilizado o comando:
...
Os próximos passos deverão ser realizados pelo Administrador Municipal, descrito no item 3.11.3 deste capítulo.
...
> ![](media/image59.png) **ATENÇÃO**: este relatório traz dados importantes sobre a consistência dos cadastros realizados no CNES em relação aos dados necessários para um bom funcionamento no sistema, portanto, é altamente recomendado que seja analisado antes de prosseguir.
...
> ![](media/image74.png) ATUALIZAÇÃO**:** A partir da versão 4.0, é possível editar os recursos dos perfis- padrão. Na migração para novas versões que possuam alterações nos perfis padrão do sistema, os perfis- padrão editados receberão os novos recursos definidos na nova versão, e não terão recursos removidos.
...
> ![](media/image74.png) ATUALIZAÇÃO: A partir da versão 4.0, os links dos centralizadores estaduais estarão configurados automaticamente nas instalações municipais para os estados que formalizarem a solicitação via ofício ao Ministério da Saúde.
...
O módulo de Gestão municipal agrupa funcionalidades gerais, são elas: Compartilhamento de prontuário, Configuração da agenda padrão dos profissionais, Configuração RNDS, Grupos de exame e Tipo de serviço.
...
Também é possível configurar a duração padrão do agendamento (duração padrão da consulta). As lotações que possuem CBOs com duração do agendamento customizado não não utiliza a configuração de duração padrão. Para configurar a duração padrão do agendamento, informe o valor em minutos e clique em Salvar.

![](media/image219.png)

## 3.11.3 Configurações do Acesso à RNDS

O módulo de Configurações do Acesso à Rede Nacional de Dados em Saúde (RNDS) permitirá que os profissionais de saúde que utilizam o PEC acessem os dados hospedados nessa plataforma nacional por meio do botão "Acessar RNDS", presente no módulo de atendimento.

Para concluir a configuração, iniciada pelo Administrador da Instalação (descrita no item 3.1.6 deste capítulo) o Administrador Municipal deverá clicar na aba "Configuração RNDS" e selecionar o certificado, digitar a senha e clicar no botão ![](media/image220.png).

![](media/image220.png)

Em caso de sucesso na operação o sistema apresentará a data de inclusão do certificado, o operador que o habilitou e a validade do certificado.

![](media/image221.png)

Caso haja necessidade de desabilitar este certificado, utilize o botão ![](media/image222.png). Ao desabilitar o certificado digital o botão "Acessar RNDS" não será mais apresentado no PEC.
...
- integração com Cadastro da Atenção Básica (via CDS ou aplicativo e-SUS APS Território)
...
O módulo de **Agendamento Online** foi desenvolvido conjuntamente entre o Departamento de Atenção Básica - DAB e o Departamento de Informática do SUS - DATASUS, a fim de facilitar o acesso dos cidadãos aos serviços de Atenção Básica em todo o país. Dessa forma, foi realizado o desenvolvimento da ferramenta de Agenda Online no PEC e adequações no aplicativo "ConecteSUS" para a efetivação desta funcionalidade.

Este módulo está disponível para as equipes de Atenção Básica que utilizam o PEC do e-SUS APS, a partir da versão 3.1, e foi pensando levando em consideração avanços tecnológicos e no aumento de acesso constante da população a aparelhos de *smartphones*, dessa forma tendo acesso a aplicações *mobile*.
...
> ![](media/image59.png) **ATENÇÃO**: A funcionalidade de agendamento de consulta na Atenção Básica por meio do aplicativo "ConecteSUS" está disponível apenas para usuários que fazem parte do território de equipes de atenção básica. Para tanto é necessário que o cidadão esteja cadastrado no território da equipe através dos cadastros individual e domiciliar.
>
> ![](media/image58.png) **NOTA**: A mudança de configuração da agenda pode ser realizada a qualquer momento, porém não altera os horários agendados previamente.
>
> ![](media/image84.png) **DICA**: Antes de realizar a configuração da Agenda Online realize uma conversa prévia com toda a equipe, para pactuação de estratégia de abertura desse formato de agendamento, a disponibilização de horários pode ser gradual. Atente- se para informar a população sobre esse possibilidade de agendamento.

![btModAgenda.png](media/image267.png)
...
Após confirmar o Cancelamento, a visualização do agendamento será apresentado como na Figura 5.5.
...
- Passo 2. Clique no horário que deseja reservar;
...
Após salvo, a reserva do horário será apresentada na agenda, junto com os outros agendamentos.
...
É possível realizar a impressão da agenda dos profissionais individualmente, para um determinado dia da agenda.

Para isto basta selecionar a data do dia desejado, por meio do calendário, e clicar no botão "imprimir" ![](media/image303.png).
...
Além dos atendimentos agendados para os profissionais da unidade de saúde, é possível a inclusão no sistema dos cidadãos, que procuram o serviço de saúde por demanda espontânea. Para mais informações sobre demanda espontânea, ver [**CAB 28**](http://189.28.128.100/dab/docs/publicacoes/geral/miolo_CAP_28.pdf) **- ** **[Acolhimento à Demanda Espontânea](http://189.28.128.100/dab/docs/publicacoes/geral/miolo_CAP_28.pdf) - Volume I**.
...
6\. Selecionar no campo "Lote/Fabricante", o número de fabricação do lote que se encontra, geralmente, no frasco do imunobiológico, assim como o nome do fabricante da vacina. Se o lote/fabricante não estiver previamente cadastrado no sistema é possível realizar a inserção do lote e fabricante no sistema a partir da opção "+ Adicionar novo". Após será aberta uma tela conforme a figura 6.10:

![](media/image354.png)

Figura 6.14 - Cadastrar lote
![](media/image355.png)

a)No campo "Cadastrar lote" digite o número do Lote, o nome do Fabricante e a Data de validade contida no frasco/embalagem do imunobiológico. Observe que todos esses campos são de preenchimento
obrigatório.

b)Caso o Lote, Fabricante e a Data de validade que foi adicionado no campo "Cadastrar lote" já esteja cadastrado no sistema irá aparecer a mensagem "Este lote de imunobiológico já está cadastrado", conforme a figura 6.11. Nesta situação, clique em "OK" e após na opção ![](media/image356.png) que removerá os campos para cadastro de lote e voltará exibir o combo "Lote/Fabricante".

Figura 6.15. Mensagem de Alerta
![](media/image357.png)

7\. Dando continuidade, selecione a opção desejada nos combos "Via de administração" e "Local de aplicação". Caso seja necessário, descreva as observações no campo com este destino.
...
9\. Ainda é possível verificar as informações sobre o registro ao clicar no quadro da dose da vacina administrada (Figura 6.12).

Figura 6.16 - Visualização de Imunobiológico
![](media/image359.png)

> ![](media/image84.png) **DICA:** Para aumentar a segurança na aplicação dos imunobiológicos o sistema emite alertas quando da tentativa de registro de uma dose que não seja recomendada para o cidadão (Figura 6.13).

Figura 6.17 - Confirmação de registro de dose advertida
![](media/image360.png)
...
4\. Os combos para o registro do "Imunobiológico" e "dose" aparecerão conforme a seleção realizada pelo profissional de saúde na tela do quadro de vacinação (Figura 6.8), não sendo possível a sua alteração.
...
4\. Ao finalizar o registro clique no botão "SALVAR" e o quadro de vacinação aparecerá com a dose aprazada para uma data futura, conforme figura 6.12

Figura 6.19 - Calendário Vacinal com doses de vacinas aprazadas
![](media/image364.png)

5\. Ainda é possível verificar as informações sobre o registro ao clicar no quadro da dose da vacina aprazada (Figura 6.15).
...
- ![](media/image380.png) **Problemas/Condições e Alergias:** ferramenta que permite o registro e gestão da lista de problemas ou outras condições de saúdeo idadão, além do registro de história pregressa do cidadão relacionado a alergias e/ou a reação adversa;
...
> ![](media/image74.png) **ATUALIZAÇÃO:** A partir da versão 4.1 é possível acessar os dados clínicos do cidadão em atendimento no ambiente da Rede Nacional de Dados em Saúde (RNDS) clicando no botão ![](media/image388.png). Este botão só está disponível após a sua configuração, descrita no Capítulo 03 - Administração.

- **Acessar RNDS:** este botão permite acessar os dados clínicos do cidadão em atendimento no ambiente da Rede Nacional de Dados em Saúde (RNDS). Ao clicar será aberta uma nova aba no navegador com os registros clínicos ocorridos em outros estabelecimentos de saúde.
...
![subjetivo.png](media/image399.png)
Fonte: SAPS/MS.
...
- Passo 3. Clique no botão ![Uploaded by Awesome ScreenshotExtension](media/image402.png) para concluir a inserção do código;
...
- clique no botão "Imprimir" ![](media/image423.png) para que seja apresentada a tela com o atestado em modo de impressão;

- clique no botão "Visualizar" ![](media/image424.png) para que os dados do atestado sejam apresentados somente para visualização;

- clique no botão "Excluir" ![](media/image425.png) para retirar um atestado da listagem. O sistema solicitará a confirmação da exclusão;

- clique no botão "Editar" ![](media/image426.png) para apresentar os dados do atestado em modo de edição. Após as alterações, clique no botão "Salvar".
...
- Passo 5. Para finalizar clique no botão "Fechar" ![](media/image456.png){width="1.2083333333333333in" height="0.3645833333333333in"}.
...
- Passo 4. Para imprimir basta selecionar no botão ![](media/image467.png){width="0.25in" height="0.3125in"} e em seguida aparecerá uma tela com as orientações prescritas.
...
- **imprimir** ![](media/image423.png): realizar a impressão da solicitação. O padrão utilizado obedece às premissas do SISREG (Figura 6.60). A impressão, além dos dados da solicitação, traz o campo da contra referência, na qual pode ser preenchido pelo profissional de saúde do serviço referenciado, como forma de devolver à UBS as informações importantes sobre a situação de saúde do cidadão encaminhado;

Figura 6.70 - Modelo de Guia de Encaminhamento
![](media/image473.png)

- **visualizar** ![](media/image424.png): visualizar as informações da solicitação;

- **editar** ![](media/image426.png): editar as informações da solicitação. Esta opção não estará mais disponível após a finalização do atendimento;

- **excluir** ![](media/image425.png): excluir a solicitação. Esta opção só estará disponível durante o próprio atendimento.
...
![btAntecedentes.png](media/image493.png)
...
Esta ferramenta não está mais presente na versão 3.1. Agora é possível visualizar os registros feitos pelo sistema com CDS, por meio, da ferramenta histórico do cidadão (Figura 6.76).
...
- clique no botão "Visualizar" ![http://dab.saude.gov.br/portaldab/esus/manual_pec_1.3/manual/Cap06/images/image00.png](media/image424.png) para que os dados da solicitação de exames sejam apresentados somente para visualização;

- clique no botão "Editar" ![http://dab.saude.gov.br/portaldab/esus/manual_pec_1.3/manual/Cap06/images/image02.png](media/image426.png) para alterar algum dado da solicitação de exames. Após as alterações, clique no botão "Salvar";

- clique no botão "Excluir" ![http://dab.saude.gov.br/portaldab/esus/manual_pec_1.3/manual/Cap06/images/image01.png](media/image425.png) para apagar uma solicitação de procedimento.
...
![Fornecimento.png](media/image552.png)
...
Ao clicar no bloco de uma das faixas etárias a ser avaliada abrirá a tela com os marcos esperados para aquela faixa, conforme podemos observar na avaliação dos marcos da faixa etária do 1º mês (Figura
6.121).
...
a)**Provável atraso no desenvolvimento:** quando o perímetro cefálico é \< - 2 Z escores ou \> +2 Z escores ou presença de 3 ou mais alterações fenotípicas ou ausência de 1 ou mais reflexos/posturas habilidades para a faixa etária anterior;

b)**Alerta para o desenvolvimento:** ausência de 1 ou mais reflexos/ posturas/habilidades para a sua faixa etária ou todos os reflexos/posturas/habilidades para a sua faixa etária estão presentes, mas existe 1 ou mais fatores de risco, e

c)**Desenvolvimento normal:** todos os reflexos/posturas/habilidades presentes para a faixa etária.
